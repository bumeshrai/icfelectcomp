...
class Division extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('zoneId, divisionname', 'required'),
			array('zoneId', 'numerical', 'integerOnly'=>true),
			array('divisionname', 'length', 'max'=>25),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('divisionId, zoneId, divisionname', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'divisionId' => array(self::HAS_MANY, 'Casesheet', 'division'),
			'zoneId' => array(self::BELONGS_TO, 'Railways', 'zoneId'),
		);
	}
